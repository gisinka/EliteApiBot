    ...
    public Embed BuildEmbed(bool isRussian = true)
    {
        return isRussian
            ? new EmbedBuilder()
                ...
                .AddField("Тег эскадры", $"[{Tag}]({string.Format(Constants.ShortLink, Tag)})")
            ...
            : new EmbedBuilder()
                ...
                .AddField(nameof(Tag), $"[{Tag}]({string.Format(Constants.ShortLink, Tag)})")
                .AddField(nameof(Members), Members)
                .AddField(nameof(Owner), Owner)
                .AddField(nameof(Platform), Platform)
                .AddField(nameof(CreationDate), CreationDate.ToString(Constants.DateTimeFormat))
                .AddField(nameof(Power), Power)
                .AddField(nameof(SuperPower), SuperPower)
